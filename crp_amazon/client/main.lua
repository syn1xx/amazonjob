...
AddEventHandler('esx_amazonjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_amazonjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()    
    CurrentAction = nil
    CurrentActionMsg = ''
end)
...
function donnerlapaye()
	...
	if vievehicule >= 1 then
		if livraisonTotalPaye == 0 then
			...
			TriggerServerEvent("esx_amazonjob:pay", amount)
	...
	while true do
		...
		if IsJobTrucker() then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_amazonjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_amazonjob:hasExitedMarker', lastZone)
			end
